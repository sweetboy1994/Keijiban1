	...
	<div class="limiter">
        ...
        メール　ACTIVE
                ...
                @endif
                    ...
                    <div class="card-footer bg-transparent"><span class="font-weight-bold"> {{ __('続行する前に、確認リンクでメールを確認してください。') }}
                    {{ __('メールが届かない場合') }}, </span>  </div>
                    <div class="card-footer bg-transparent"><form class="d-inline" method="POST" action="{{ route('verification.resend') }}">
                        @csrf
                        <button type="submit" class="btn btn-link p-0 m-0 align-baseline">{{ __('別のリクエストするには、ここをクリックしてください') }}</button>.
                </form>
                    </div>
